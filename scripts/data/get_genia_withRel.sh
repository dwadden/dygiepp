# Adapted to format the GENIA relations annotations to use with the ner and coref datasets
# Adapted by Serena G. Lotreck 

# Note: If this works, change above comment and submit PR

# Download the GENIA relations dataset and preprocess into the JSON form required by
# DyGIE++.
# Usage: From main project folder, run `bash scripts/data/get_genia_withRel.sh`
# Most of the challenge here is in aligning the GENIA coref annotations with the
# relation and ner annotations.
...
rel_dir=$raw_dir/GENIAcorpus_BioNLP2011_REL
...
download_raw() {
    ...
    # Download the relation data.
    # Relation annotation is already split into training, dev and test
    # Potentially problematic if it doesn't align with the folds from sutd, may need to merge and split
    # again
    wget http://www.nactem.ac.uk/GENIA/current/GENIA-corpus/Relation/GENIA_relation_annotation_training_data.tar.gz \
        -P $raw_dir

wget -nv http://www.nactem.ac.uk/GENIA/current/GENIA-corpus/Relation/GENIA_relation_annotation_development_data.tar.gz \
        -P $raw_dir

    wget http://www.nactem.ac.uk/GENIA/current/GENIA-corpus/Relation/GENIA_relation_annotation_test_data.tar.gz \
        -P $raw_dir
    ...
    tar -xf $raw_dir/GENIA_relation_annotation_training_data.tar.gz -C $rel_dir
    tar -xf $raw_dir/GENIA_relation_annotation_development_data.tar.gz -C $rel_dir
    tar -xf $raw_dir/GENIA_relation_annotation_test_data.tar.gz -C $rel_dir
